...
public final class RunQueries {
    ...
    public static String query(final String query) throws BaseXException {
        if(query.equals(null)||query.trim().equals("")) {
            return "BASEX-01:no-query-executed";
        }
            final XQuery xQuery = new XQuery(query);
            final String resultStr = xQuery.execute(context);
     ...
     * @throws QueryException if an error occurs while evaluating the query
     */
    public static String process(final String query) throws QueryException {
     ...
     * @throws QueryException if an error occurs while evaluating the query
     */
    public static String iterate(final String query) throws QueryException {
        // Create a query processor
        try(QueryProcessor proc = new QueryProcessor(query, context)) {
     ...
     * @throws IOException if an error occurs while serializing the results
     */
    public static void serialize(final String query) throws QueryException, IOException {
        // Create a query processor
        try(QueryProcessor proc = new QueryProcessor(query, context)) {
